...
<p align='center'> <img class='noshade' src='./playgroundoutput.png' width=75%> </p>
...
<p align='center'> <img class='noshade' src='./testoutput.png' width=75%> </p>
...
<p align='center'> <img class='noshade' src='./modifiedoutput.png' width=75%> </p>
...
In the next part, the program assigns the memory address of `x` to the pointer `p`. Then, it prints out four lines with different format specifiers and arguments passed into the `printf()` function. First, it normally prints out the value inside the variable x with `printf(" x = %d \n", x);`{.C}. Then, it again prints out the value of `x`, but this time using the dereference operator on pointer `p`: `printf("*p = %d\n", *p)`{.c}, which basically means it's telling the program to "print the value that's stored by the variable that's pointed to by `p`".

The next two lines print the *memory address* of the variable `x`, instead of its value. The former references the address directly by using `&x` - essentially saying "print the address of `x`", while the latter uses the value of the pointer `p`, as we have assigned the address of `x` to it.

Next, the value of `x` is changed using the dereference operator on pointer `p` - effectively saying "go to the variable pointed to by `p`, then change it's value to 20". This is why on the next `printf()`{.C} statement, 20 is printed instead of whatever was input by the user earlier.
